...
module.exports.onStart = async ({ event, api }) => {
  ...
  try {
    let bg_img = await jimp.read("https://i.imgur.com/Lor62xE.jpeg");
    ...
    bg_img.resize(1030, 587).composite(circleOne.resize(300, 300).circle(), 125, 145).composite(circleTwo.resize(300, 300).circle(), 625, 155);
    const filePath = `./gf_${user1}_${user2}.png`;
    await bg_img.writeAsync(filePath);
    api.sendMessage(
      {
        body: `╔⏤⏤⏤╝❮Tatek bot❯╚⏤⏤⏤╗\n\nদুঃখ দিলে প্রেম দিলে না.. নিখুঁত ভাবে কাঁদিয়ে বললে আর কেঁদোনা..!!\n\n╔⏤⏤⏤╝❮❮Tatek bot❯❯╚⏤⏤⏤╗\n\n${mentions[user1] || 'User 1'} & ${mentions[user2] || 'User 2'}`,
